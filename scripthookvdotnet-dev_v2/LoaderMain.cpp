...
{
	static Collections::Generic::List<MethodInfo ^> InitMethods, TickMethods, KeyboardMethods, PresentMethods;
};
...
void ManagedInit()
{
	...
	LoaderData::PresentMethods.Clear();
	...
	for each (String ^filename in IO::Directory::EnumerateFiles(directory, "ScriptHookVDotNet*.dll"))
	{
		...
		if (main && main->IsAbstract)
		{
			...
			LoaderData::PresentMethods.Add(main->GetMethod("Present", BindingFlags::Public | BindingFlags::Static));
		}
	}
...
{
	IntPtr ^managedPtr = gcnew IntPtr(swapchain);

	array<Object ^> ^args = gcnew array<Object ^>(1) { managedPtr };

	//WinForms::MessageBox::Show(String::Concat("ManagedD3DCall\n\nSwapchain address is ", managedPtr->ToString()), "Community Script Hook V .NET");

	for each (MethodInfo ^method in LoaderData::PresentMethods)
	{
		method->Invoke(nullptr, args);
	}
}
